...
struct PhasePlot_parameters
    type::Symbol
    p_range::Tuple
    h_range::Tuple
    N::Int64
    z::Any
    function PhasePlot_parameters(;type::Symbol,p_range::Tuple,h_range::Tuple,N::Int64,z::Any)
        @assert p_range[1] <= p_range[2]
        @assert h_range[1] <= h_range[2]
        new(type,p_range,h_range,N,z)
    end
end

function PhasePlot(plot_param::PhasePlot_parameters;fluid=set_fluid)
    if isnothing(fluid)
        throw(error("Fluid not selected"))
    end
    @unpack type,p_range,h_range,N,z = plot_param

    if type != :PH || type != :TS
        throw(error("The type selected for phase plots is not :TS or :PH"))
    end

end


function PhasePlotClapeyron(plot_param::PhasePlot_parameters;fluid=set_fluid)
    @assert fluid isa EoSModel
    @unpack type,p_range,h_range,N,z = plot_param
    @assert z isa AbstractVector
    p = collect(range(p_range[1],p_range[2],N)); h = collect(range(h_range[1],h_range[2],N))

    mix_phase_identification(fluid,p,h) = PhaseIdentification(fluid,p,h,z)

    phase = zeros(N,N)

    for i in eachindex(phase,1)
        for j in eachindex(phase,2)
            try
                phase[i,j] = mix_phase_identification(fluid,p[i],h[j])
            catch
                phase[i,j] = NaN
            end
        end
